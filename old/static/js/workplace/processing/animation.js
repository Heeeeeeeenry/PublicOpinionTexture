...
const ProcessingAnimation = {
    ...
    async initStart(header, leftPanel, rightPanel) {
        ...
        // 检查是否需要中断
        if (processingController?.initStartAnimationInterrupt) {
            return;
        }
        ...
        if (processingController) {
            processingController.initStartAnimationOver = true;
        }
    ...
    initStartInterrupt() {
        ...
        if (processingController && !processingController.initStartAnimationOver) {
            ...
            // 2. 清空页面内容
            const container = processingController.container;
            if (container) {
                container.innerHTML = '';
            }

            // 3. 设置中断标志
            processingController.initStartAnimationInterrupt = true;
        }
    },
    ...
    async reenterAnimation(container) {
        ...
        // 1. 页面头部轻微淡入
        tl.add({
            targets: container.querySelector('.processing-header'),
            opacity: [0.7, 1],
            translateY: [-10, 0],
            duration: 400
        })
